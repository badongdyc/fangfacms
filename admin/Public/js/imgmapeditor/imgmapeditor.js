...
function imgmap(config) {
	...
	this.config.hreflinks = []; //jameskerr
	//the links value of the area href,like : [{'id':1,'link':'http://www.fangfacms.com','title':'方法数码'}]
...
imgmap.prototype.setup = function(config) {
	...
	if (!this.config.maplang) {
		this.config.maplang = this.detectLanguage();
	}
	this.loadScript(this.config.baseroot + 'lang_' + this.config.maplang + '.js');
...
imgmap.prototype.onLoad = function(e) {
	...
	if (typeof imgmapStrings == 'undefined') {
		if (this.cntReloads++ < 5) {
			...
			this.log('Delaying onload (language ' + this.config.maplang + ' not loaded, try: ' + this.cntReloads + ')');
			return false;
		}
		else if (this.config.maplang != this.config.defaultLang && this.config.defaultLang != 'en') {
			this.log('Falling back to default language: ' + this.config.defaultLang);
			this.cntReloads = 0;
			this.config.maplang = this.config.defaultLang;
			this.loadScript(this.config.baseroot + 'lang_' + this.config.maplang + '.js');
			window.setTimeout(function () {_this.onLoad(e);} ,1200);
			return false;
		}
		else if (this.config.maplang != 'en') {
			this.log('Falling back to english language');
			this.cntReloads = 0;
			this.config.maplang = 'en';
			this.loadScript(this.config.baseroot + 'lang_' + this.config.maplang + '.js');
			window.setTimeout(function () {_this.onLoad(e);} ,1200);
			return false;
		}
			...
			for (var i=0; i<this.config.buttons.length; i++) {
				if (this.config.buttons[i] == 'add') {
					try {
						...
						img.src     = this.config.imgroot + 'imgmap_add.gif';
					...
					try {
						img = document.createElement('IMG');
						img.src     = this.config.imgroot + 'imgmap_delete.gif';
					...
					try {
						img = document.createElement('IMG');
						img.src     = this.config.imgroot + 'imgmap_zoom.gif';
					...
					try {
						img = document.createElement('IMG');
						img.src     = this.config.imgroot + 'imgmap_html.gif';
					...
					try {
						img = document.createElement('IMG');
						img.src     = this.config.imgroot + 'imgmap_clipboard.gif';
...
imgmap.prototype.loadImage = function(img, imgw, imgh) {
	...
	if (!img)
	{
		...
		{
			this.pic_container.removeChild(this.pic);
			delete this.pic;
		}
		return false;
	}
...
imgmap.prototype.getMapHTML = function() {
	var html = '<map id="'+this.getMapId()+'" name="'+this.getMapName()+'">' + this.getMapInnerHTML() + '</map><img src="'+this.mapcurimgsrc+'" usemap="#'+this.getMapName()+'" />';
...
imgmap.prototype.setMapHTML = function(map) {
	...
	for (var i=0; i<newareas.length; i++) {
		...
		if (href) {
			if (this.props[id]) {
				this.props[id].getElementsByTagName('select')[1].value = href;
		...
		if (target == '') target = '_blank'; //jamesekerr
		if (this.props[id]) {
			this.props[id].getElementsByTagName('select')[2].value = target;
		}
...
imgmap.prototype.togglePreview = function() {
	...
	if (this.viewmode === 0) {
		...
		this.viewmode = 1;
		this.i_preview.src = this.config.imgroot + 'imgmap_edit.gif';
		...
		//change preview button
		this.viewmode = 0;
		this.i_preview.src = this.config.imgroot + 'imgmap_zoom.gif';
...
imgmap.prototype.addNewArea = function() {
		...
		if (this.form_container)
		{
			...
			var temp = (id+1) + ': <input type="hidden"  name="img_id" class="img_id" value="' + id + '" readonly="1"/>';//jameskerr
			temp+= '<input type="radio" name="img_active" class="img_active" id="img_active_'+id+'" value="'+id+'">';
			temp+= this.strings.SHAPE+': <select name="img_shape" class="img_shape">';
			temp+= '<option value="rectangle" >'+this.strings.RECTANGLE+'</option>';
			temp+= '<option value="circle"    >'+this.strings.CIRCLE+'</option>';
			temp+= '<option value="polygon"   >'+this.strings.POLYGON+'</option>';
			temp+= '</select>';
			temp+= '<input type="hidden" name="img_coords" class="img_coords" value="">';//jameskerr
			//temp+= '  Href: <input type="text" name="img_href" class="img_href" style="width:200px;" value="">';
			//jameskerr{
			temp+= this.strings.HREF+': <select name="img_href" class="img_href" class="img_coords" style="width:200px;">';
			temp+= '<option value="" >'+this.strings.PLEASE_SELECT+'</option>';
			for(var i=0; i<this.config.hreflinks.length; i++)
			{
				temp+= '<option value="'+this.config.hreflinks[i].url+'" >'+this.config.hreflinks[i].title+'</option>';
			}
			temp+= '</select>';
			//jameskerr}
			temp+= '<!--Alt: --><input type="hidden" name="img_alt" class="img_alt" value="">';
			temp+= '<!--Target: --><select name="img_target" style="display:none;">';//jameskerr
			...
			this.addEvent(this.props[id].getElementsByTagName('select')[1], 'change', this.img_area_blur.bind(this));//jameskerr(blur)
			this.addEvent(this.props[id].getElementsByTagName('select')[2], 'change', this.img_area_blur.bind(this));//jameskerr
			if (this.isSafari) {
...
imgmap.prototype._putlabel = function(id) {
	...
	try {
		if (!this.config.label) {
			...
			label = label.replace(/%n/g, String(id+1));//jameskerr
