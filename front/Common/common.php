	...
	if ($num > $perpage) {
		...
		for($i = $from; $i <= $to; $i ++) {
			if ($i != $curr_page) {
				...
				$multipage .= '<li class="now"><a href="javascript:;"  style="color:#fff;">'.$i.'</a></li>';
			}
		}
		//下一页
		if ($curr_page>0 && $curr_page<$pages) {
			$multipage .= '<li class="noma"><a href="'. str_replace ( '{page}', ($curr_page+1), $mpurl ).'" >'.$config['next'].'</a></li>';
		} else {
		    $multipage .= '<li class="noma"><a href="'. str_replace ( '{page}', $curr_page, $mpurl ).'"  >'.$config['next'].'</a></li>';
		}
		$multipage .= '</ul>';
		$multipage .= '</div>';
	}
