...
class FguestbookAction extends FbaseAction {	
	...
	public function add() {
		...
		if (!empty($in['content'])) {
			...
			if (empty($in['title'])) {
				$this->message(L('请输入标题！'), $this->forward);
			}	
			...
			$data = array(
				'catid'			=>	(int)$in['catid'],
				'title'			=>	htmlspecialchars($in['title']),
				'content'		=>	htmlspecialchars($in['content']),		
	...
	public function newadd() {
		...
		if (!empty($in['content'])) {
			...
			if (empty($in['sheng'])) {
				$this->message(L('请选择省份！'), $this->forward);
			}
			if (empty($in['shi'])) {
				$this->message(L('请选择市！'), $this->forward);
			}
			if(empty($in['address'])){
				$this->message(L('请输入地址！'), $this->forward);
			}
			if(! is_numeric ($in['telphone'])){
				$this->message(L('请正确填写电话号码！'), $this->forward);
			}
			$data = array(
					'catid'			=>	(int)$in['catid'],
					'title'			=>	htmlspecialchars($in['title']),
			...
			//$this->assign('seo', $seo); //meta信息
//		$this->display($this->_category_data['setting']['template']['show']);
			$this->message(L("内容不能为空    ，请稍后再试"));
		}
	}
	
	
	public function newdd() {
		$in = &$this->in;
		array_map('trim', $in);
		if (!empty($in['content'])) {
			//验证数据来源
